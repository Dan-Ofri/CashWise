...
function analyzeBudgetSnapshot() {
    ...
    if (savingsRate < 0.1) recs.push('💡 נסה להעלות חיסכון ל-10%-20% מההכנסה.');
    if (wants > income * 0.3) recs.push('🎯 הוצאות רצוניות גבוהות — שאף לכלל 50/30/20.');
    if (needs > income * 0.5) recs.push('🏠 הוצאות צרכים גבוהות — בדוק דיור/תחבורה.');
    if (recs.length === 0) recs.push('✅ התקציב נראה מאוזן ביחס לכלל 50/30/20.');
...
function analyzeEmergency() {
    ...
    const monthsTarget = parseFloat(document.getElementById('ef-months')?.value) || 3;
    const target = monthlyExpense * monthsTarget;
    const monthsToGoal = monthlySaving > 0 ? Math.ceil(target / monthlySaving) : Infinity;
    const adequate = monthsTarget >= 3;
    
    const recs = [];
    if (!monthlyExpense) recs.push('🔎 הוסף הוצאות חודשיות כדי לחשב קרן חירום מותאמת.');
    if (adequate && isFinite(monthsToGoal) && monthsToGoal <= 12) {
        recs.push('🛡️ אתה בדרך טובה ליעד קרן החירום תוך שנה.');
    }
    if (monthlySaving < monthlyExpense * 0.15) {
...
export function buildMentorResponse(question) {
    ...
    parts.push(`
        <div class="tip-box info">
            ...
            חיסכון: ${(budget.savingsRate * 100).toFixed(1)}% | 
